...
class CreateWorkAreaDialog(QDialog):
    ...
    def run_console_command(self):
        not_empty_images_paths = [path for path in self.images_paths if path]
        console_command = f"python {self.settings.value('python_script')} {self.settings.value('result_folder')} {self.settings.value('weights')} {self.work_area.work_area_path} {' '.join(not_empty_images_paths)}"
    ...
    def load_shapefile(self):
        ...
        green_shapefile_path = os.path.join(result_folder, 'green.shp')

        if os.path.exists(green_shapefile_path):
            self.load_vector_layer(green_shapefile_path, "Green Layer")
            print(f"green.shp loaded from {green_shapefile_path}")
        else:
            print(f"green.shp not found in {result_folder}")
